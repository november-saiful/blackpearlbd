...
export interface CoverflowCarouselProps {
  ...
  /**
   * Fired when the centred card is clicked, or when Enter/Space is pressed on the
   * rack. Clicking a card beside the centre brings it to the centre instead.
   */
  onCardClick?: (index: number) => void;
  /** Scale factor applied to the centred card so it stands out from the rack. */
  centerScale?: number;
...
export function CoverflowCarousel({
  ...
  /** Set once a gesture travels far enough to be a drag, so it ends as no click. */
  ...
  const paint = React.useCallback(() => {
    ...
    cardRefs.current.forEach((card, index) => {
      ...
      // The centred card is scaled up so it stands out from the rack; side cards
      // stay at full size (the perspective recession already makes them read smaller).
      const scale = distance === 0 ? centerScale : 1;
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A little travel is a shaky hand, not a drag: only past that does releasing
    // over a card stop counting as a click on it.
    if (!movedRef.current && Math.abs(event.clientX - drag.x) > 6) movedRef.current = true;
        ...
        <div
          ...
          onKeyDown={(event) => {
            if (event.key === "ArrowLeft") {
              ...
              event.preventDefault();
              nudge(1);
            } else if (onCardClick && (event.key === "Enter" || event.key === " ")) {
              // The rack is one tab stop, so Enter/Space opens what is centred.
              event.preventDefault();
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onClick={() => {
                  // A drag that happens to end over a card is not a click on it.
                  if (movedRef.current) return;
                  if (index === selected) onCardClick?.(index);
                  // A click beside the centre is a request to see that card.
                  else goTo(index);
                }}
                className={cn(
                  "absolute left-1/2 top-0 aspect-square overflow-hidden rounded-2xl bg-muted shadow-xl will-change-transform",
                  index === selected && onCardClick && "cursor-zoom-in",
                  cardClassName,
                )}
                style={{ width: "var(--cf-card)" }}
                title={index === selected && onCardClick ? "View photo" : undefined}
              >
                <img
                  src={slide.src}
                  alt={slide.alt}
