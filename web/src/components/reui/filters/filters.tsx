...
export function Filters<V = unknown, O = unknown>({
  ...
  React.useEffect(() => {
    ...
    if (draft.ruleId) {
      ...
      const id = nextId()
      const defaultOperator = draft.operator ?? ""
      addRule(
        createFilterRule<V>({
          id,
          path: draft.path,
      ...
      // When the field has a default operator that takes a value, skip the
      // operator step and open the value editor directly.
      const field = getFilterField(latest.current.index, draft.path)
      const operators = field ? latest.current.resolveOperators(field) : []
      const resolvedOp = getFilterOperator(operators, defaultOperator)
      const segment =
        defaultOperator && operatorTakesValue(resolvedOp) ? "value" : "operator"
      focusStore.set({ id, segment, autoOpen: true })
    }
    dispatchDraftRaw({ type: "close" })
