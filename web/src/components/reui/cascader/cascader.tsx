...
function CascaderTrigger({
  ...
  const setTrigger = React.useCallback(
    (node: HTMLButtonElement | null) => {
      ...
      // `RefObject.current` is readonly in the React 18 types, so the mutable
      // shape is asserted for the object-ref branch only.
      else if (ref)
...
function CascaderContent({
  ...
  const setPopup = React.useCallback(
    (node: HTMLDivElement | null) => {
      ...
      if (typeof ref === "function") ref(node)
      // See `setTrigger`: readonly `current` in the React 18 types.
      else if (ref)
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          className={cn(
            ...
            /* The three `[var(...)]` bounds are what keep the panel ON SCREEN at
               phone widths: the positioner publishes how much room it found on
               the side it chose, and an UNBOUNDED panel simply overflows the
               viewport - it is measured too late for the positioner to flip it,
               so the rows above the fold cannot be clicked at all. */
            "group/combobox-content relative flex max-h-[var(--available-height)] max-w-[var(--available-width)] min-w-[var(--anchor-width)] origin-[var(--transform-origin)] flex-col",
            className
          )}
          {...props}
        />
    ...
    body = (
      ...
      >
        ...
        <button
          ...
          className="text-foreground hover:bg-accent focus-visible:ring-ring/50 rounded-md px-2 py-0.5 font-medium outline-none transition-colors focus-visible:ring-2"
        >
          {labels.retry}
        </button>
      </span>
