...
function CascaderSubmenuTrigger({
  ...
  onClick,
  disabled,
  /* Named, so the spread below cannot reintroduce it: `ref` is the one prop a
     spread clobbers rather than merges. */
  ref: consumerRef,
  ...props
  ...
  /**
   * The flyout needs the row's node twice over: it is what the close handler
   * focuses and what `Positioner` anchors to. Base UI hands the node to one
   * `ref` and no more, so a consumer's own ref is chained by hand, exactly as
   * `CascaderTrigger` chains it.
   */
  ...
  return (
    <PopoverPrimitive.Trigger
      /* Base UI declares this trigger's `ref` as the intersection of a callback
         ref and an object ref whose `current` is NOT nullable. No ref built
         from the React 18 types satisfies that, and `setRowRef` above is typed
         at its source, so only the prop itself is silenced. */
      ref={setRowRef as never}
      id={triggerId}
      ...
      >
        <PopoverPrimitive.Popup
          ...
          className={cn(
            ...
            "flex max-w-[var(--available-width)] min-w-48 flex-col gap-0.5 outline-none",
            CASCADER_LIST_PAD_CLASS,
            "p-[var(--cascader-list-pad,4px)]",
            className
          )}
