...
/**
 * The child element `Slot` should be handed, with this part's own content
 ...
 * `Slot` that content directly deleted the part: Radix clones its child to
...
function CascaderBack({
  ...
  return (
    <Comp
      ...
      onClick={() => popLevel()}
      className={cn(
        "text-muted-foreground hover:bg-accent hover:text-accent-foreground focus-visible:ring-ring/50 flex shrink-0 items-center justify-center rounded-md outline-none transition-colors focus-visible:ring-2",
    ...
    >
      ...
      {segment.type === "ellipsis" ? (
        ...
        <button
          ...
          className="hover:text-foreground focus-visible:ring-ring/50 max-w-32 truncate rounded-sm outline-none transition-colors focus-visible:ring-2"
        >
          {segment.node.label}
        </button>
      ) : (
  ...
  return (
    ...
    >
      ...
      <ComboboxPrimitive.Input
        ...
        className={cn(
          "placeholder:text-muted-foreground h-full w-full min-w-0 flex-1 bg-transparent px-1.5 text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50",
          backVisible && "ps-0",
          className
        )}
        {...props}
...
function CascaderValue({
  ...
  // See `cascaderSlotChild`: `Slot` cannot clone the Fragment the default
