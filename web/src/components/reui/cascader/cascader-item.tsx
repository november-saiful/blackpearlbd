...
const ROW_THEME_CLASS =
  "data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground [&:not([data-variant=destructive])]:data-[highlighted]:[&_*]:text-accent-foreground [&_svg:not([class*='size-'])]:size-4 rounded-md py-1"
...
const ROW_SHELL_CLASS = `${ROW_THEME_CLASS} relative flex w-full cursor-default items-center outline-none select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 ${ROW_INSET_CLASS}`
...
// be invisible. `!` and `[&_*]:` for the `CHECKBOX_MARK_CLASS` reason.
const AFFORDANCE_HOVER_CLASS =
  "transition-[background-color,box-shadow,color] hover:bg-background hover:!text-foreground hover:[&_*]:!text-foreground hover:shadow-sm"
...
const INDICATOR_MARK_CLASS = "!text-foreground [&_*]:!text-foreground"
...
const CascaderItem = React.memo(function CascaderItem({
  ...
  const checkbox = (
    <span
      ...
      className="border-input [&:where([data-selected]_*)]:bg-primary [&:where([data-selected]_*)]:border-primary [&:where([data-selected]_*)]:text-primary-foreground [&:where([data-indeterminate]_*)]:bg-primary [&:where([data-indeterminate]_*)]:border-primary [&:where([data-indeterminate]_*)]:text-primary-foreground flex size-4 shrink-0 items-center justify-center rounded-[4px] border transition-colors"
    >
      ...
      {indeterminate ? (
        ...
        <CheckIcon data-slot="cascader-item-tick" className={cn(
                              CHECKBOX_MARK_CLASS,
                              "size-3 opacity-0 [&:where([data-selected]_*)]:opacity-100"
                            )} />
      )}
    </span>
  )
