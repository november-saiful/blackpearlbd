 ...
 */
/* Tailwind 3 syntax: `w-(--var)` and the `not-first:` variant are v4 only, and
 * a class the compiler does not know generates NOTHING - a column laid out with
 * no width at all and no divider between columns. */
const PANEL_CLASS = `flex w-[var(--cascader-column-width)] shrink-0 flex-col overscroll-contain [&:not(:first-child)]:w-[calc(var(--cascader-column-width)_+_1px)] [&:not(:first-child)]:border-border/60 [&:not(:first-child)]:border-l ${CASCADER_LIST_HEIGHT_CLASS}`
    ...
    emptyBody = (
      <>
        ...
        <button
          ...
          className="text-foreground hover:bg-accent focus-visible:ring-ring/50 rounded-md px-1 font-medium outline-none transition-colors focus-visible:ring-2"
        >
          {labels.retry}
        </button>
      </>
